...
protected:
	...
	virtual void preTraverse(Node* u, int& counter);
	virtual void postTraverse(Node* u, int& counter);
	virtual void inTraverse(Node* u, int& counter);
	virtual void getLE(Node* u, DLList<T>& list, const T& x);
public:
	BinarySearchTree();
...
template<class Node, class T>
void BinarySearchTree<Node, T>::preTraverse() {
	int counter = 1;
	preTraverse(r, counter);
}

template<class Node, class T>
void BinarySearchTree<Node, T>::preTraverse(Node *u, int& counter) {
	if (u == nil) return;
	u->pre_order = counter++;
	preTraverse(u->left, counter);
	preTraverse(u->right, counter);
}

template<class Node, class T>
void BinarySearchTree<Node, T>::postTraverse() {
	int counter = 1;
	preTraverse(r, counter);
}

template<class Node, class T>
void BinarySearchTree<Node, T>::postTraverse(Node *u, int& counter) {
	if (u == nil) return;
	postTraverse(u->left, counter);
	postTraverse(u->right, counter);
	u->post_order = counter++;
}

template<class Node, class T>
void BinarySearchTree<Node, T>::inTraverse() {
	int counter = 1;
	preTraverse(r, counter);
}

template<class Node, class T>
void BinarySearchTree<Node, T>::inTraverse(Node *u, int& counter) {
	if (u == nil) return;
	inTraverse(u->left, counter);
	u->in_order = counter++;
	inTraverse(u->right, counter);
}
...
template<class Node, class T>
void BinarySearchTree<Node, T>::getLE(Node* u, DLList<T>& list, const T& x) {
	if (u == nil)
		return ;
	getLE(u->left, list, x);
	if (u->x < x)
